...
class Board(gd.BoardGame):
    ...
    def create_game_objects(self, level = 1):
        ...
        self.board.add_unit(data[0]-3,0,3,2,classes.board.MultiImgSprite,"",white,os.path.join("schemes",scheme,"next.png"),0,frame_flow = [0,1],frame_count=2,row_data=[2,1])
        self.next_btn = self.board.ships[-1]

        self.board.add_unit(0,2,2,2,classes.board.ImgShip,"",white,os.path.join("schemes",scheme,"c_quati.png"),0)
        self.poli_btn = self.board.ships[-1]

        self.board.add_unit(0,4,2,2,classes.board.ImgShip,"",white,os.path.join("schemes",scheme,"c_tria.png"),0)
    ...
    def pick_shape(self):
        ...
        if self.check_sizes:
            ...
            if self.chosen in [1,6]: #non special case trapeziums and right trapeziums
                ...
                self.size_instr.value  = self.lang.d["size_instr_0"] % (self.shape_sizes[0],self.shape_sizes[1],self.shape_sizes[2])
            ...
            if self.check_sizes:
                if self.shape_id in [10,11,12,13,14,15,16,17]: #any triangle
                    if self.shape_id in [11, 12] and self.tria_variant == 1:
                        ...
                        if self.is_near(self.dst[0], self.shape_sizes[0]) and self.is_near(self.heights[0], self.shape_sizes[1]) or self.is_near(self.dst[1], self.shape_sizes[0]) and self.is_near(self.heights[1], self.shape_sizes[1]) or self.is_near(self.dst[2], self.shape_sizes[0]) and self.is_near(self.heights[2], self.shape_sizes[1]):
                            ok = True
                elif self.shape_id in [1,6,7,8]:
                    if self.trapezium_variant == 1:
                        if ((self.is_near(self.dst[0], self.shape_sizes[0]) and self.is_near(self.dst[2],self.shape_sizes[1]) or self.is_near(self.dst[2], self.shape_sizes[0]) and self.is_near(self.dst[0], self.shape_sizes[1])) and self.is_near(self.heights[1],self.shape_sizes[2])):
                            ok = True
                    ...
                    if (self.is_near(self.dst[0], self.shape_sizes[0]) and self.is_near(self.dst[1], self.shape_sizes[0])):
                        ok = True
                elif self.shape_id == 5:
                    if (self.is_near(self.dst[0], self.shape_sizes[0]) and self.is_near(self.dst[1], self.shape_sizes[1])) or (self.is_near(self.dst[0], self.shape_sizes[1]) and self.is_near(self.dst[1], self.shape_sizes[0])):
                        ok = True
    ...
    def collinear4(self, p):
        if self.collinear(p[0],p[1],p[2]) or self.collinear(p[1],p[2],p[3]) or self.collinear(p[2],p[3],p[0]) or self.collinear(p[3],p[0],p[1]):
            return True
        else:
            return False

    def collinear_all(self, p):
        if self.collinear(p[0],p[1],p[2]) and self.collinear(p[1],p[2],p[3]) and self.collinear(p[2],p[3],p[0]) and self.collinear(p[3],p[0],p[1]):
            return True
        else:
            return False
    ...
    def check_quadrilateral(self,points):
        ...
        if (self.is_parallel(v[0],v[2]) and self.not_intersecting(points[1],points[3],points[0],points[2])) or (self.is_parallel(v[1],v[3]) and self.not_intersecting(points[0],points[2],points[1],points[3])):
            ...
            if self.is_parallel(v[0],v[2]) and self.is_parallel(v[1],v[3]):
                ...
                if abs(self.side_len(self.vectors[0]) - self.side_len(self.vectors[1])) < 0.01:
                    ...
                    self.shape_id = 5
            elif self.is_orthogonal(v[0],v[1]) or self.is_orthogonal(v[1],v[2]) or self.is_orthogonal(v[2],v[3]) or self.is_orthogonal(v[3],v[0]):
                self.name = self.lang.d["right_trapezium"]
                self.shape_id = 6
            elif abs(self.side_len(self.vectors[0]) - self.side_len(self.vectors[2])) < 0.01  or abs(self.side_len(self.vectors[1]) - self.side_len(self.vectors[3])) < 0.01:
        ...
        if self.shape_id == 0:
            ...
            if self.is_parallel(v[0],v[2]):
                ...
                l = ["a = " + dsts[1],"b = " + dsts[3],"c = " + dsts[0],"d = " + dsts[2],"h = "+str(hs[0]),"p = " + dsts[4],"q = " + dsts[5]]
            else:
                self.trapezium_variant =1
                l = ["a = " + dsts[0],"b = " + dsts[2],"c = " + dsts[1],"d = " + dsts[3],"h = "+str(hs[1]),"p = " + dsts[4],"q = " + dsts[5]]
        elif self.shape_id == 2:
            l = ["a = "+dsts[0],"b = "+dsts[1],"","h1 = "+hs[0],"h2 = "+hs[1],"p = "+dsts[4],"q = "+dsts[5],""]
    ...
    def t_iso(self):
        return abs(self.side_len(self.vectors[0]) - self.side_len(self.vectors[1])) < 0.01 or abs(self.side_len(self.vectors[1]) - self.side_len(self.vectors[2])) < 0.01 or abs(self.side_len(self.vectors[2]) - self.side_len(self.vectors[0])) < 0.01

    def t_acute(self,v):
        return self.angle(v[0],v[1]) < 0 and self.angle(v[1],v[2]) < 0 and self.angle(v[2],v[0]) < 0

    def q_acute(self,v):
        return self.angle(v[0],v[1]) < 0 and self.angle(v[1],v[2]) < 0 and self.angle(v[2],v[3]) < 0 and self.angle(v[3],v[0]) < 0
